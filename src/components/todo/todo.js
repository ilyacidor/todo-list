...
export default class Todo extends Component {
  ...
  changeTimerState = (id, state) => {
    this.setState(({ todoData }) => {
      const idx = todoData.findIndex((el) => el.id === id)
      const oldItem = todoData[idx]
      const newItem = { ...oldItem, timerIsRun: state }
      const newArray = [...todoData.slice(0, idx), newItem, ...todoData.slice(idx + 1)]
      return {
        todoData: newArray,
      }
  ...
  startTimer = (id) => {
    ...
    const indicatorId = setInterval(() => {
      this.setState(({ todoData }) => {
        const idx = todoData.findIndex((el) => el.id === id)
        const oldItem = todoData[idx]
        if (!oldItem.timerIsRun) return clearInterval(indicatorId)
        let minutes = Number(oldItem.time.slice(0, 2))
        let seconds = Number(oldItem.time.slice(3, 5))
        if (minutes === 0 && seconds === 0) return clearInterval(indicatorId)
        if (seconds > 0) {
          seconds--
        } else {
          minutes--
